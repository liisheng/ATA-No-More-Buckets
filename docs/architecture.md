...
```mermaid
flowchart LR
  T[Tenant text / image / video / voice] --> TG[Telegram Bot API webhook]
...
Telegram tenant intake is a persisted draft workflow: `/report` creates one expiring `TelegramDraft`, each ordered text/caption/photo/video/voice update appends to it, and Submit uses the draft ID as the single incident idempotency key. Inline **✅ Submit report**, **➕ Add / edit items**, **↩️ Undo last**, and **🗑 Cancel** callbacks are authorized against the paired chat and claimed by Telegram `update_id`; `/submit`, `/undo`, `/clear`, and `/cancel` are equivalent command paths. Item keys combine update/message/file/media-group identifiers, and summaries use the draft revision so legitimate counter updates are not suppressed. A successful draft keeps a short-lived submitted tombstone so repeated Submit taps return the original incident rather than creating another one. Vendor callbacks and guided price/ETA/completion replies use the same deterministic action surface. Completion requires an after-photo, work summary, and confirmed final price before the evidence gate can run.
...
Media is stored behind a `MediaStore` port. The API only returns media descriptors and draft/incident-scoped URLs after checking that the asset ID belongs to that draft or incident; it never returns arbitrary GCS paths, Telegram file IDs, chat IDs, or storage URIs. Video responses support byte ranges for browser playback. The console polls incidents, communications, and media every second, selecting the newest incident by persisted `updated_at` for a simple restart-safe live feed. Draft inspection is available only with local adapters and is disabled on the cloud deployment.
