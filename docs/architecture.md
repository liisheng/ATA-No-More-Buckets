...
Media is stored behind a `MediaStore` port. The API only returns media descriptors and draft/incident-scoped URLs after checking that the asset ID belongs to that draft or incident; it never returns arbitrary GCS paths, Telegram file IDs, chat IDs, or storage URIs. Video responses support byte ranges for browser playback. The console polls drafts, incidents, communications, and media every second, selecting the newest incident by persisted `updated_at` for a simple restart-safe live feed.
